...
async function processBarberShop(barberShopId: string): Promise<AutoPaymentResult> {
  try {
    ...
    if (!isFreeTrial) {
      ...
      const { data: freeTrialPeriods } = await supabase
    ...
    logError(error as Error, `Erro ao processar barbearia ${barberShopId}:`);
    return {
      success: false,
 ...
 */
export async function runAutomaticPaymentCreation(): Promise<AutoPaymentResult[]> {
  try {
    ...
    logError(error as Error, 'Erro na execução automática de pagamentos:');
    return [{
      success: false,
...
/**
 * Função para testar a criação automática em uma barbearia específica
 * Útil para testes e depuração
 */
export async function testAutomaticPaymentForBarberShop(barberShopId: string): Promise<AutoPaymentResult> {
  return await processBarberShop(barberShopId);
} 