...
-- Função para verificar se o usuário é admin de uma barbearia (movida para schema public)
CREATE OR REPLACE FUNCTION public.is_barber_shop_admin(barber_shop_id uuid)
RETURNS boolean AS $$
BEGIN
  RETURN EXISTS (
...
CREATE POLICY "Usuários podem criar apenas uma barbearia" ON barber_shops
...
CREATE POLICY "Ver pagamentos de assinaturas da própria barbearia" ON subscription_payments
  FOR SELECT USING (
    client_subscription_id IN (
      ...
      JOIN barber_shops bs ON bs.id = c.barber_shop_id
      WHERE bs.admin_id = auth.uid()
    )
  );
